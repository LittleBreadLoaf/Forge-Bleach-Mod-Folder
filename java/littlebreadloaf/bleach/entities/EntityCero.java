...
public class EntityCero extends Entity implements IProjectile
{
	...
	public EntityCero(World var1, double var2, double var4, double var6)
	{
		super(var1);
		this.renderDistanceWeight = 10.0D;
		this.setSize(0.5F, 0.5F);
	...
	public EntityCero(World var1, EntityLiving var2, EntityLiving var3, float var4, float var5)
	{
		super(var1);
		this.renderDistanceWeight = 10.0D;
	...
	public EntityCero(World var1, EntityLivingBase var2, float var3)
	{
		super(var1);
		this.renderDistanceWeight = 10.0D;
		this.shootingEntity = var2;
	...
	public EntityCero(World var1, EntityMenosGrande menos, EntityPlayer target, float var3) 
	{
		super(var1);
		this.renderDistanceWeight = 10.0D;
		this.shootingEntity = menos;

		this.setSize(0.5F, 0.5F);
		double hyp1 = Math.sqrt((menos.posX - target.posX)*(menos.posX - target.posX) + (menos.posZ - target.posZ)*(menos.posZ - target.posZ));
		
		this.setLocationAndAngles(menos.posX, menos.posY + (double) menos.getEyeHeight(), menos.posZ, (float)Math.atan((menos.posX - target.posX)/(menos.posZ - target.posZ)), (float)Math.atan(menos.height/hyp1));
		this.posX -= (double) (MathHelper.cos(this.rotationYaw / 180.0F * (float) Math.PI) * 0.16F);
		this.posY -= 0.10000000149011612D;
		this.posZ -= (double) (MathHelper.sin(this.rotationYaw / 180.0F * (float) Math.PI) * 0.16F);
		this.setPosition(this.posX, this.posY, this.posZ);
	...
	public void onUpdate()
	{
		...
		} else
		{
			for (int ar5 = -1; ar5 <= 1; ++ar5)
			{
				for (int ar6 = -1; ar6 <= 1; ++ar6)
				{
					for (int ar7 = -2; ar7 <= 2; ++ar7)
					{
						...
						if (!this.worldObj.isRemote && this.ticksExisted > 3)
						{
							
						if (ar11 != null && ar11 != Blocks.bedrock && ar11 != Blocks.obsidian && ar11 != Blocks.portal && ar11 != BleachBlocks.reiatsuBlock)
						{
			...
			for (var9 = 0; var9 < var6.size(); ++var9)
			{
				...
				if (var11.canBeCollidedWith() && (var11 != this.shootingEntity))
				{
					var10 = 0.3F;
			...
			if (var4 != null && this.ticksExisted > 2)
			{
				...
				worldObj.createExplosion(this, this.posX, this.posY, this.posZ, 2, this.worldObj.getGameRules().getGameRuleBooleanValue("mobGriefing"));
