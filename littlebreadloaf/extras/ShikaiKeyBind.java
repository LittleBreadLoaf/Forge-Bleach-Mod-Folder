		...
		private Minecraft mc = Minecraft.getMinecraft();
         ...
         public void keyDown(EnumSet<TickType> types, KeyBinding kb, boolean tickEnd, boolean isRepeat)
         {
                ...
                if(heldItem != null )
                {
             	   ...
             	   {
             		  props.syncThings(Items.zanpakuto.itemID);
             		  heldItem.itemID = Items.zanpakuto.itemID;
                     
             	   }
             	   else if(heldItem.itemID == Items.quincybow.itemID)
             	   {
             		   props.syncThings(Items.quincybow.itemID);
             		   ItemStack Pendant = new ItemStack(Items.quincypendant, 1, 1);
             		   heldItem.itemID = Pendant.itemID;
             		   heldItem.setItemDamage(0);
             	   }
             	   else if(heldItem.itemID == Items.quincyweb.itemID)
             	   {
             		   props.syncThings(Items.quincyweb.itemID);
